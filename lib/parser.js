...
const mapDot = (str = "") => {
  if (/^(POST|GET|PUT|DELETE|OPTIONS)/.test(str)) {
    ...
    return {
      method,
      method1: toUpperCase(method.toLocaleLowerCase()),
      method2: method.toLocaleLowerCase(),
      url,
    };
  }
...
const parser = (schemaCode = "", type) => {
  ...
  const out = schemas.map((item) => {
    ...
    if (item.name && item.name.kind == "Name") {
      ...
      out.upperName = toUpperCase(out.name);
    }
    if (item.fields) {
      out.fields = item.fields.map((field) => {
        ...
        if (field.arguments) {
          ...
          if (arg) {
            sub.input = parseType(loadTypeName(arg, {}), type);
          } else {
            sub.input = parseType({}, type);
          }
        }
  ...
  out.forEach((sub) => {
    ...
    sub.fields.forEach((item) => {
      ...
      if (item.isHandle && item.type && data[item.type]) {
        ...
        const sub = JSON.parse(JSON.stringify(data[item.type]));
        item.type = sub.name;
      }
    });
  });
